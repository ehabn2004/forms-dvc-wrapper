...
public class TitleBackgroundColor extends FormsGraphPropertyHandler {
    @Override
    public boolean handleProperty(String sParams, BincsoftBean bean) {
        if (super.handleProperty(sParams, bean)) {
            Color col = ColorCodeRegistry.getColorCode(sParams);
            if (col != null) {
                log("SET_TITLE_BACKGROUND: setting " + sParams + " as a new background color");
                graph.getGraph().getDataviewTitle().setBackground(col);
            } else {
                log("SET_TITLE_BACKGROUND: " + sParams +
                                   " passed is not a valid color name or RGB value");
            }
        } else {
            log("SET_TITLE_BACKGROUND: no color specified");
        }
        return true;
    }
}
